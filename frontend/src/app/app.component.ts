...
@Component({
  ...
  template: `
    <nav class="nav">
      <div class="nav-bar">
        <a class="brand" routerLink="/ask" (click)="close()">🛡️ AGM Sentinel</a>
        <button class="nav-toggle" type="button" (click)="toggle()"
                [attr.aria-expanded]="menuOpen()" aria-label="Toggle navigation">
          {{ menuOpen() ? '✕' : '☰' }}
        </button>
      </div>
      ...
      <div class="nav-links" [class.open]="menuOpen()">
        ...
        @if (auth.isModerator()) {
          ...
          <a routerLink="/setup" routerLinkActive="active" (click)="close()">Setup</a>
  ...
  styles: [`
    ...
    .brand { color: var(--accent); font-weight: 800; text-decoration: none; font-size: 16px; white-space: nowrap; }
    .nav-toggle {
      display: none; background: none; border: none; color: var(--text);
      font-size: 22px; line-height: 1; padding: 4px 8px; cursor: pointer; width: auto;
    }
    .nav-links { display: flex; align-items: center; gap: 16px; padding: 0 24px 12px; }
    ...
    @media (max-width: 760px) {
      ...
      .nav-links a { padding: 10px 0; width: 100%; border-bottom: 1px solid #33415533; }
      .nav-spacer { display: none; }
      .nav-user { padding: 8px 0; }
    }
  `],
})
